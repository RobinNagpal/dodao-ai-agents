...
from koala_gains.structures.public_equity_structures import CriteriaLookupList, TickerReport, Sector, IndustryGroup, \
    get_ticker_file_key, CriterionReportDefinitionItem, IndustryGroupCriteria, IndustryGroupCriterion, \
...
def get_criterion_definition(sector_id: int, industry_group_id: int, criteria_key: str, report_key: str) -> IndustryGroupCriterion:
    criteria_lookup_list = get_criteria_lookup_list()

    matching_criteria = get_matching_criteria_lookup_item(criteria_lookup_list, sector_id, industry_group_id)

    if matching_criteria.customCriteriaFileUrl is None:
        raise Exception(f"Custom criteria file not found for sector and industry group: {sector_id}, {industry_group_id}")
    ...
    criterion = next(criterion for criterion in industry_group_criteria.criteria if criterion.key == criteria_key)
    return criterion

def get_criteria_report_definition(sector_id: int, industry_group_id: int, criteria_key: str, report_key: str) -> CriterionReportDefinitionItem:
    criteria_lookup_list = get_criteria_lookup_list()
    ...
    industry_group_criteria = IndustryGroupCriteria.model_load_json(json_response)
    criterion = next(criterion for criterion in industry_group_criteria.criteria if criterion.key == criteria_key)
    report_definition = next(report for report in criterion.reports if report.key == report_key)

    return report_definition


def save_criteria_evaluation(ticker: str, criterion_key: str, report_key: str, data: any) -> str:
    report = get_ticker_report(ticker)
    report_definition = get_criteria_report_definition(report.selectedSector.id, report.selectedIndustryGroup.id, criterion_key, report_key)

    criterion_report_key = get_criterion_report_key(ticker, criterion_key, report_key, report_definition.outputType)

    return upload_to_s3(data, criterion_report_key, content_type= "text/plain" if report_definition.outputType == "Text" else "application/json")
