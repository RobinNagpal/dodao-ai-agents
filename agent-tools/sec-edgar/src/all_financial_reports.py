...
def get_statement_of_equity(ticker: str, period_of_report: str) -> str:
    ...
    """
    try:
        raw_text = specific_report_text(ticker, "equity_statement", fetch_raw_content = True)
...
def filter_older_columns(df, period_of_report: str):
    """
    ...
    columns_to_include: ColumnsToInclude = structured_llm.invoke([HumanMessage(content=user_prompt)])

    print(f"Columns to include: {columns_to_include}")

    # Drop only existing columns to prevent KeyErrors
    columns_to_exclude = [col for col in df.columns if col not in columns_to_include.columns_to_include]
...
def fetch_from_raw_html(ticker: str, report_type: str, period_of_report: str) -> str:
    ...
    if not raw_content:
        raise ValueError(f"No raw content found for ticker '{ticker}' and report type '{report_type}'.")
...
def cross_check_income_statement(ticker, df_based_report: str, raw_html_based_report: str, period_of_report: str) -> str:
    """
...
def get_xbrl_financials(ticker: str, force_refresh: bool = False) -> str:
    ...
    for statement_name, df in statements.items():
        if df is not None and not df.empty:
            ...
            if force_refresh and statement_name == "Income Statement":
                raw_html_based_report = fetch_from_raw_html(ticker, statement_name.lower().replace(" ", "_"), period_of_report)
                cleaned_df = cross_check_income_statement(ticker, cleaned_df.to_markdown(), raw_html_based_report, period_of_report)
                markdown_output = f"### {statement_name}\n" + cleaned_df
                print('Cross-checked and cleaned DataFrame:\n', cleaned_df)
    ...
    equity_statement = get_statement_of_equity(ticker, period_of_report)
    if equity_statement:
        latest10Q_financial_statements += "\n\n\n\n" + equity_statement
