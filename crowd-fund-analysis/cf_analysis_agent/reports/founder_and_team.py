...
def generate_team_performance_report(state: AgentState, profile_information: str) -> StructuredReportResponse:
    ...
    return structured_report_response(
        state.get("config"),
        "detailed_founder_and_team_report",
        prompt
    )
...
def create_founder_and_team_report(state: AgentState) -> None:
    ...
    try:
        ...
        startup_info: StartupAndTeamInfoStructure = find_startup_info(state.get("config"), content_of_crowdfunding_url)
        team_members_tables: list[str] = []
        for member in startup_info.team_members:
            member_info_as_table = generate_team_member_report(startup_info.startup_name, member)
